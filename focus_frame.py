"""
FocusFrame - AI-powered focus and distraction monitoring.
...
class PresenceDetector:
    """Handles presence detection using YOLO, motion, and smoothing."""
    ...
    def update_score(self, person_detected, motion_detected, current_time):
        """Update presence smoothing score."""
    ...
    def check_presence_change(self, prev_score, current_time):
        """Check if presence state has changed (away/return)."""
        # Return detection
...
class DistractionMonitor:
    """Monitors for distracting applications and windows."""
    ...
    def check_distractions(self):
        """Check if active window matches distraction keywords."""
...
class PhonePopup:
    """Handles phone detection popup window."""
    
    def __init__(self):
        self.active = False
    
    def show(self, on_dismiss_callback=None):
        """Show popup requiring 'Im back at working' to dismiss."""
        if self.active:
            return
...
class ReportGenerator:
    """Generates HTML session reports."""
...
class FocusFrameEngine:
    """Main monitoring engine orchestrating all components."""
    ...
    def _run_work_period(self, duration_seconds):
        """Run a timed work period with pause support."""
        cap = cv2.VideoCapture(0)
        if not cap.isOpened():
    ...
    def _display_frame(self, results):
        """Display annotated camera frame."""
        try:
    ...
    def _display_frame_with_timer(self, results, remaining_seconds, paused):
        """Display frame with Pomodoro timer overlay."""
        try:
            annotated_frame = results[0].plot()
...
def setup_pomodoro_interactive(app):
    """Interactive Pomodoro configuration."""
    print("\n" + "-"*50)
